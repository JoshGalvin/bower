...
Package.prototype.download = function () {
  ...
  // if Tarball download and unpack, otherwise...
  var file = '';
  ...
  tmp.dir(function (err, tmpPath) {
    req.get(src, function (res) {
      switch(this.assetType) {
        case ".tar.gz":
        case ".tgz":
          res.pipe(zlib.createGunzip()).pipe(tar.Extract({path: this.path = tmpPath}));
          break;
        case ".tar":
          res.pipe(tar.Extract({path: this.path = tmpPath}));
          break;
        default: 
          res.pipe(fs.createWriteStream(path.join((this.path = tmpPath), 'index' + this.assetType)));
      }
      res.on('end', function () {
