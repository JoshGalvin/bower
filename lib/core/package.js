  ...
  tmp.dir(function (err, tmpPath) {
    req.get(src, function (res) {
      switch(this.assetType) {
        case ".tar.gz":
        case ".tgz":
          res = res.pipe(zlib.createGunzip()).pipe(tar.Extract({path: this.path = tmpPath}));
          this.assetType = this.assetUrl = undefined;
          break;
        case ".tar":
          res = res.pipe(tar.Extract({path: this.path = tmpPath}));
          this.assetType = this.assetUrl = undefined;
          break;
        default: 
          res = res.pipe(fs.createWriteStream(path.join((this.path = tmpPath), 'index' + this.assetType)));
      }
      res.on('end', function () {
